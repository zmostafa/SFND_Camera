...
void projectLidarToCamera2()
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        // Filter LiDAR Points
        float maxX = 25.0; // object far away from veihcle
        float maxY = 6.0; // Object not facing the camera either to the right or left
        float minZ = -1.4; // point close to road
        if(it->x > maxX || it->x < 0.0 || abs(it->y) > maxY || it->z < minZ  || it->r < 0.1){
            continue;
        }
        ...
        // cv::circle(overlay, cv::Point(x, y), 5, cv::Scalar(0, green, red), -1);
    }
